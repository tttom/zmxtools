...
from zmxtools.optical_design.material import (MaterialLibrary, Material, FunctionMaterial, PolynomialMaterial,
                                              MaterialResistance)
...
class AgfMixin:
    ...
    def __init__(self, command: Command):
        ...
        (formula_index, glass_code, self.__refractive_index_d, self.__constringence, exclude_sub, status_index,
         self.production_frequency) = to_length([as_float(_) for _ in self.command.words[1:]], 7, value=0)
        ...
        self.thermal_coefficients = to_length(
            self.command['TD', 0].numbers if 'TD' in self.command else list[float](), 6, value=0).real
        ...
        wavelength_limits = np.sort(to_length(self.command['LD', 0].numbers, 2, value=0))[::-1] * 1e-6 \
            if 'CD' in self.command else asarray([np.inf, 0])  # Convert micrometers to meters, in reverse order
    ...
    def adjust_refractive_index_function(self,
                                         ...
                                         temperature: array_type, pressure: array_type) -> array_type:
        """
        ...
        if self.specified_wavenumber_transmission_thickness.shape[0] > 0:  # by interpolationv
            ...
            transmission = lambda _: _.real * 0 + 1.0
            transmission_thickness = lambda _: _.real * 0 + 1.0
