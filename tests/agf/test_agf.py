...
from tests.agf import test_agf_files, test_directory
from tests.zmx import log
from zmxtools import agf
from zmxtools.optical_design.material import CiddorAir, SimpleAir, Vacuum
...
def test_vacuum():
    """Check whether the vacuum _material_ responds as expected."""
    vacuum = Vacuum()
    npt.assert_almost_equal(vacuum.complex_refractive_index(wavelength=633e-9), 1.0, decimal=16,
                            err_msg='Vacuum refractive index is not returned correctly.',
                            )
    npt.assert_almost_equal(vacuum.complex_refractive_index(wavelength=[488e-9, 532e-9, 633e-9]), 1, decimal=16,
                            err_msg='Vacuum refractive index is not returned correctly.',
                            )
    npt.assert_almost_equal(vacuum.extinction_coefficient(wavelength=[488e-9, 532e-9, 633e-9]), 0, decimal=16,
                            err_msg='Vacuum extinction coefficient is not returned correctly.',
                            )


def test_CiddorAir():
    """Test the model for air. This is important because other refractive indices are given with respect to air."""
    air = CiddorAir(pressure=101_325, temperature=20 + 273.15, relative_humidity=0, co2_mole_fraction=450e-6)
    npt.assert_almost_equal(air.complex_refractive_index(wavelength=633e-9), 1.0002718, decimal=9,
                            err_msg='Air refractive index is returned incorrectly.',
                            )
    air.pressure = 1e-12
    npt.assert_almost_equal(air.complex_refractive_index(wavelength=633e-9), 1.0, decimal=16,
                            err_msg='Air refractive index at 0 pressure is returned incorrectly.',
                            )
    air.pressure = 0
    ...
    air = CiddorAir(pressure=100e3, temperature=50 + 273.15, relative_humidity=0, co2_mole_fraction=450e-6)
    ...
    npt.assert_almost_equal(air.complex_refractive_index(wavelength=633e-9), 1.00033758, decimal=9,
                            err_msg='Air refractive index is returned incorrectly.',
                            )
    air = CiddorAir(pressure=120e3, temperature=50 + 273.15, relative_humidity=1, co2_mole_fraction=450e-6)
    npt.assert_almost_equal(air.complex_refractive_index(wavelength=633e-9), 1.000287924, decimal=9,
                            err_msg='Air refractive index is returned incorrectly.',
                            )
    air = CiddorAir(pressure=100_000, temperature=30 + 273.15, relative_humidity=0.5, co2_mole_fraction=500e-6)
    npt.assert_almost_equal(air.complex_refractive_index(wavelength=633e-9), 1.00025863, decimal=8,
                            err_msg='Air refractive index is returned incorrectly.',
                            )
    air = CiddorAir(pressure=101_325, temperature=-40 + 273.15, relative_humidity=0.5, co2_mole_fraction=450e-6)
    npt.assert_almost_equal(air.complex_refractive_index(wavelength=633e-9), 1.000342056, decimal=9,
                            err_msg='Air refractive index is returned incorrectly.',
                            )
    air = CiddorAir(pressure=140e3, temperature=-2 + 273.15, relative_humidity=0.5, co2_mole_fraction=450e-6)
    npt.assert_almost_equal(air.complex_refractive_index(wavelength=633e-9), 1.000406109, decimal=9,
                            err_msg='Air refractive index is returned incorrectly.',
                            )
    air = CiddorAir(pressure=140e3, temperature=-0.1 + 273.15, relative_humidity=0.5, co2_mole_fraction=450e-6)
...
def test_SimpleAir():
    """Check whether the simplified formula for air is computed correctly."""
    ...
    air.pressure = 1e-12
    npt.assert_almost_equal(air.complex_refractive_index(wavelength=633e-9), 1, decimal=16,
                            err_msg='Air refractive index at 0 pressure is returned incorrectly.',
                            )
    air.pressure = 0
    npt.assert_almost_equal(air.complex_refractive_index(wavelength=633e-9), 1, decimal=16,
...
def test_from_file():
    ...
    for agf_file_path in test_agf_files:
        ...
        assert len(material_library) >= 1, (f'Too few materials ({len(material_library)}) found ' +
                                            f'in library {material_library.name} at {agf_file_path}!',
                                            )
        ...
        for material in material_library:
            ...
            if material.name == 'N-BK7':  # 2 in SCHOTT
                ...
                log.info(f'Analysing {material.name} of ' +
                         f'type {material.__class__.__name__} from {material_library.name}...',
                         )
                ...
                npt.assert_almost_equal(
                    material.refractive_index_C, 1.5143, decimal=4,
                    err_msg=f'{material.name} n_C is returned incorrectly as {material.refractive_index_C}.',
                )
                npt.assert_almost_equal(
                    material.refractive_index_d, 1.5168, decimal=4,
                    err_msg=f'{material.name} n_d is returned incorrectly as {material.refractive_index_d}.',
                )
                npt.assert_almost_equal(
                    material.refractive_index_F, 1.5224, decimal=4,
                    err_msg=f'{material.name} n_F is returned incorrectly as {material.refractive_index_F}.',
                )
                npt.assert_almost_equal(
                    material.constringence, 64.17, decimal=2,
                    err_msg=f'{material.name} Abbe number / constringence is returned incorrectly ' +
                            f'as {material.constringence}.',
                )
                npt.assert_almost_equal(material.refractive_index(wavelength=500e-9), 1.5214, decimal=4,
                                        err_msg='Refractive index is incorrect.',
                                        )
                npt.assert_almost_equal(material.refractive_index(wavenumber=2 * np.pi / 500e-9), 1.5214, decimal=4,
                                        err_msg='Refractive index is incorrect when using wavenumber.',
                                        )
                npt.assert_almost_equal(material.refractive_index(angular_frequency=2 * np.pi / 500e-9 * const_c),
                                        1.5214, decimal=4,
                                        err_msg='Refractive index is incorrect when using angular frequency.',
                                        )
                npt.assert_almost_equal(material.transmittance(0.025, wavelength=500e-9), 0.994, decimal=5,
                ...
                npt.assert_almost_equal(material.extinction_coefficient(wavelength=500e-9), 9.5781e-9, decimal=13,
                                        err_msg='extinction coefficient κ is incorrect.',
                                        )
                npt.assert_almost_equal(material.extinction_coefficient(wavenumber=2 * np.pi / 500e-9), 9.5781e-9,
                                        decimal=13,
                                        err_msg='extinction coefficient κ is incorrect when using wavenumber.',
                                        )
                npt.assert_almost_equal(material.extinction_coefficient(angular_frequency=2 * np.pi / 500e-9 * const_c),
                                        9.5781e-9, decimal=13,
                                        err_msg='extinction coefficient κ is incorrect when using angular frequency.',
                                        )
                ...
                npt.assert_almost_equal(material.transmittance(10e-3, wavelength=500e-9), 0.9976, decimal=5,
                ...
                # Check pressure
                npt.assert_equal(material.pressure, 101.13e3,
                                 f'Expected default pressure to be 101,130 Pa, not {material.pressure} Pa.',
                                 )
            ...
            if material.name == 'S-BAH54':  # 1 in OHARA
                tested_material_type_dict[material.__class__].append(agf_file_path)
                log.info(f'Analysing {material.name} of type {material.__class__.__name__} ' +
                         f'from {material_library.name}...',
                         )
                ...
                npt.assert_almost_equal(
                    material.constringence, 42.17, decimal=2,
                    err_msg=f'{material.name} Abbe number / constringence is returned incorrectly ' +
                            f'as {material.constringence}.',
                )
            ...
            if material.name == 'H-FK61' and material_library.name == 'cdgm.agf':  # 2
                ...
                npt.assert_almost_equal(material.transmittance(10e-3, wavelength=633e-9), 0.998, decimal=5,
                                        err_msg=f'{material.name} transmittance incorrect for 1cm',
                                        )
            ...
            if material.name == 'CALCITE':
                ...
                npt.assert_almost_equal(material.refractive_index(wavelength=0.5e-6), 1.666, decimal=3,
                                        err_msg=f'{material.name} n is returned incorrectly.',
            ...
            if material.name == 'YVO4':
                ...
                npt.assert_almost_equal(material.refractive_index(wavelength=1e-6), 1.9603, decimal=3,
                                        err_msg=f'{material.name} n is returned incorrectly.',
                                        )  # TODO: Can this be more accurate?
            ...
            if material.name == 'AGGAS2':
                ...
                npt.assert_almost_equal(material.refractive_index(wavelength=1e-6), 2.4583, decimal=2,
                                        err_msg=f'{material.name} n is returned incorrectly.',
                                        )  # TODO: Can this be more accurate?
            ...
            if material.name == 'J-FK5':
                ...
                npt.assert_almost_equal(material.refractive_index_d, 1.48749, decimal=6,
                                        err_msg=f'{material.name} n is returned incorrectly.',
                                        )
    ...
    all_agf_materials = (agf.SchottAgfMaterial, agf.Sellmeier1AgfMaterial, agf.Sellmeier3AgfMaterial,
