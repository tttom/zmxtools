...
class TestPolynomial(unittest.TestCase):
    def test_zernike_index(self):
        ...
        npt.assert_almost_equal(tip(np.array([0.0, 0.5, 1.0]), np.pi/2),
                                np.array([0.0, 0.0, 0.0]), 12, 'Rotated tip fit failed',
                                )
    ...
    def test_zernike_index_array(self):
        ...
        npt.assert_array_almost_equal(ab4(1), np.array([1, 2, 0, np.sqrt(3)]), 12,
                                      'Array of aberrations failed at rho=1',
                                      )
        ...
        npt.assert_array_almost_equal(ab22(1), np.array([[1, 2], [0, np.sqrt(3)]]), 12,
                                      'Array of aberrations failed at rho=1',
                                      )
        npt.assert_array_almost_equal(ab22([[[0]], [[1]]]),
                                      np.array([[[1, 0], [0, -np.sqrt(3)]], [[1, 2], [0, np.sqrt(3)]]],
                                               ),
                                      12, 'Array of aberrations failed at rho=[0, 1]',
                                      )
        ...
        npt.assert_array_almost_equal(ab4([[0], [1]], [[0], [0]]), np.array([[1, 0, 0, -np.sqrt(3)],
                                                                             [1, 2, 0, np.sqrt(3)]],
                                                                            ),
        ...
        npt.assert_array_almost_equal(ab22([[[0]], [[1]]], 0),
                                      np.array([[[1, 0], [0, -np.sqrt(3)]], [[1, 2], [0, np.sqrt(3)]]]),
...
class TestIndexConversion(unittest.TestCase):
    ...
    def test_orders2index(self):
        npt.assert_equal(orders2index(0, 0), 0, 'Piston not converted to the correct index.')
        npt.assert_equal(orders2index(1, -1), 1, 'Tilt not converted to the correct index.')
        npt.assert_equal(orders2index(1, 1), 2, 'Tip not converted to the correct index.')
        npt.assert_equal(orders2index(2, -2), 3, 'Astigmatism-diag not converted to the correct index.')
        npt.assert_equal(orders2index(2, 0), 4, 'Defocus not converted to the correct index.')
        npt.assert_equal(orders2index(2, 2), 5, 'Astigmatism-cartesian not converted to the correct index.')
        npt.assert_equal(orders2index(3, -3), 6, 'Trefoil- not converted to the correct index.')
        npt.assert_equal(orders2index(3, -1), 7, 'Coma not converted to the correct index.')
        npt.assert_equal(orders2index(3, 1), 8, 'Coma not converted to the correct index.')
        npt.assert_equal(orders2index(3, 3), 9, 'Trefoil+ not converted to the correct index.')
        npt.assert_equal(orders2index(4, 0), 12, 'Spherical aberration not converted to the correct index.')
...
class TestNoll(unittest.TestCase):
    ...
    def test_orders2noll(self):
        npt.assert_equal(orders2noll(0, 0), 1, 'Piston not converted to the correct Noll index.')
        npt.assert_equal(orders2noll(1, 1), 2, 'Tip not converted to the correct Noll index.')
        npt.assert_equal(orders2noll(1, -1), 3, 'Tilt not converted to the correct Noll index.')
        npt.assert_equal(orders2noll(2, 0), 4, 'Defocus not converted to the correct Noll index.')
        npt.assert_equal(orders2noll(2, -2), 5, 'Astigmatism-diag not converted to the correct Noll index.')
        npt.assert_equal(orders2noll(2, 2), 6, 'Astigmatism-cartesian not converted to the correct Noll index.')
        npt.assert_equal(orders2noll(3, -1), 7, 'Coma not converted to the correct Noll index.')
        npt.assert_equal(orders2noll(3, 1), 8, 'Coma not converted to the correct Noll index.')
        npt.assert_equal(orders2noll(3, -3), 9, 'Trefoil- not converted to the correct Noll index.')
        npt.assert_equal(orders2noll(3, 3), 10, 'Trefoil+ not converted to the correct Noll index.')
        npt.assert_equal(orders2noll(4, 0), 11, 'Spherical aberration not converted to the correct Noll index.')

    def test_noll2index(self):
        npt.assert_equal(noll2index(1), 0, 'Piston not converted to the correct standard index.')
        npt.assert_equal(noll2index(2), 2, 'Tip not converted to the correct standard index.')
        npt.assert_equal(noll2index(3), 1, 'Tilt not converted to the correct standard index.')
        npt.assert_equal(noll2index(4), 4, 'Defocus not converted to the correct standard index.')
        npt.assert_equal(noll2index(5), 3, 'Astigmatism-diag not converted to the correct standard index.')
        npt.assert_equal(noll2index(6), 5, 'Astigmatism-cartesian not converted to the correct standard index.')
        npt.assert_equal(noll2index(7), 7, 'Coma not converted to the correct standard index.')
        npt.assert_equal(noll2index(8), 8, 'Coma not converted to the correct standard index.')
        npt.assert_equal(noll2index(9), 6, 'Trefoil not converted to the correct standard index.')
        npt.assert_equal(noll2index(10), 9, 'Trefoil not converted to the correct standard index.')
        npt.assert_equal(noll2index(11), 12, 'Spherical aberration not converted to the correct standard index.')
    ...
    def test_index2noll(self):
        npt.assert_equal(index2noll(0), 1, 'Piston not converted to the correct Noll index.')
        npt.assert_equal(index2noll(2), 2, 'Tip not converted to the correct Noll index.')
        npt.assert_equal(index2noll(1), 3, 'Tilt not converted to the correct Noll index.')
        npt.assert_equal(index2noll(4), 4, 'Defocus not converted to the correct Noll index.')
        npt.assert_equal(index2noll(3), 5, 'Astigmatism-diag not converted to the correct Noll index.')
        npt.assert_equal(index2noll(5), 6, 'Astigmatism-cartesian not converted to the correct Noll index.')
        npt.assert_equal(index2noll(7), 7, 'Coma not converted to the correct Noll index.')
        npt.assert_equal(index2noll(8), 8, 'Coma not converted to the correct Noll index.')
        npt.assert_equal(index2noll(6), 9, 'Trefoil- not converted to the correct Noll index.')
        npt.assert_equal(index2noll(9), 10, 'Trefoil+ not converted to the correct Noll index.')
        npt.assert_equal(index2noll(12), 11, 'Spherical aberration not converted to the correct Noll index.')
    ...
    def test_index2noll2index(self):
        test = np.arange(1000)
        npt.assert_array_equal(noll2index(index2noll(test)), test,
                               err_msg='index2noll is not the inverse of noll2index.',
                               )
...
class TestFringe(unittest.TestCase):
    ...
    def test_orders2fringe(self):
        npt.assert_equal(orders2fringe(0, 0), 1, 'Piston not converted to the correct Fringe index.')
        npt.assert_equal(orders2fringe(1, 1), 2, 'Tip not converted to the correct Fringe index.')
        npt.assert_equal(orders2fringe(1, -1), 3, 'Tilt not converted to the correct Fringe index.')
        npt.assert_equal(orders2fringe(2, 0), 4, 'Defocus not converted to the correct Fringe index.')
        npt.assert_equal(orders2fringe(2, -2), 6, 'Astigmatism-diag not converted to the correct Fringe index.')
        npt.assert_equal(orders2fringe(2, 2), 5, 'Astigmatism-cartesian not converted to the correct Fringe index.')
        npt.assert_equal(orders2fringe(3, -1), 8, 'Coma not converted to the correct Fringe index.')
        npt.assert_equal(orders2fringe(3, 1), 7, 'Coma not converted to the correct Fringe index.')
        npt.assert_equal(orders2fringe(3, -3), 11, 'Trefoil- not converted to the correct Fringe index.')
        npt.assert_equal(orders2fringe(3, 3), 10, 'Trefoil+ not converted to the correct Fringe index.')
        npt.assert_equal(orders2fringe(4, 0), 9, 'Spherical aberration not converted to the correct Fringe index.')

    def test_fringe2index(self):
        npt.assert_equal(fringe2index(1), 0, 'Piston not converted to the correct standard index.')
        npt.assert_equal(fringe2index(2), 2, 'Tip not converted to the correct standard index.')
        npt.assert_equal(fringe2index(3), 1, 'Tilt not converted to the correct standard index.')
        npt.assert_equal(fringe2index(4), 4, 'Defocus not converted to the correct standard index.')
        npt.assert_equal(fringe2index(5), 5, 'Astigmatism-diag not converted to the correct standard index.')
        npt.assert_equal(fringe2index(6), 3, 'Astigmatism-cartesian not converted to the correct standard index.')
        npt.assert_equal(fringe2index(7), 8, 'Coma not converted to the correct standard index.')
        npt.assert_equal(fringe2index(8), 7, 'Coma not converted to the correct standard index.')
        npt.assert_equal(fringe2index(9), 12, 'Trefoil not converted to the correct standard index.')
        npt.assert_equal(fringe2index(10), 9, 'Trefoil not converted to the correct standard index.')
        npt.assert_equal(fringe2index(11), 6, 'Spherical aberration not converted to the correct standard index.')
    ...
    def test_index2fringe(self):
        npt.assert_equal(index2fringe(0), 1, 'Piston not converted to the correct Fringe index.')
        npt.assert_equal(index2fringe(2), 2, 'Tip not converted to the correct Fringe index.')
        npt.assert_equal(index2fringe(1), 3, 'Tilt not converted to the correct Fringe index.')
        npt.assert_equal(index2fringe(4), 4, 'Defocus not converted to the correct Fringe index.')
        npt.assert_equal(index2fringe(5), 5, 'Astigmatism-diag not converted to the correct Fringe index.')
        npt.assert_equal(index2fringe(3), 6, 'Astigmatism-cartesian not converted to the correct Fringe index.')
        npt.assert_equal(index2fringe(8), 7, 'Coma not converted to the correct Fringe index.')
        npt.assert_equal(index2fringe(7), 8, 'Coma not converted to the correct Fringe index.')
        npt.assert_equal(index2fringe(12), 9, 'Trefoil- not converted to the correct Fringe index.')
        npt.assert_equal(index2fringe(9), 10, 'Trefoil+ not converted to the correct Fringe index.')
        npt.assert_equal(index2fringe(6), 11, 'Spherical aberration not converted to the correct Fringe index.')
    ...
    def test_index2fringe2index(self):
        test = np.arange(1000)
        npt.assert_array_equal(fringe2index(index2fringe(test)), test,
                               err_msg='index2fringe is not the inverse of fringe2index.',
                               )
