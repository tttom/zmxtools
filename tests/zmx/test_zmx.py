...
from zmxtools.optical_design.optic import CompoundElement
...
def check_optical_design(optical_design, file_path):
    ...
    assert isinstance(optical_design, zmx.ZmxOpticalDesign), f"The optical design {repr(optical_design)} is not a ZmxOpticalDesign."
    assert len(zmx_surfaces) > 0, f"No surfaces detected for the optical system in {file_path}!"
    assert len(zmx_surfaces) >= 3, f"Only {len(zmx_surfaces)} surfaces detected for the optical system in {file_path}. At least 3 expected in {file_path}."
    ...
    assert nb_stops <= 1, f"Multiple stop surfaces set! At most one of the {zmx_surfaces} surfaces should have a stop in {file_path}."
    assert nb_stops >= 1, f"No stop surface set! At least one of the {zmx_surfaces} surfaces is expected for the test lens files in {file_path}."
    assert not optical_design.source.surface.stop, "The stop should not be set at the object surface in {zmx_file_path}."
    assert not zmx_surfaces[-1].stop, "The stop should not be set at the image surface in {zmx_file_path}."
    ...
    assert all(abs(_) != math.inf for _ in distances[1:-2]), \
        ("With the exception of that of the first surface, lens elements should not be infinitely thick, only the "
         + f"object and image surfaces may have infinite thickness, not {[f'{_.distance} {_.type}' for _ in zmx_surfaces[1:-2]]} in {file_path}.")
    ...
    assert has_blackbox or any(_ != 0.0 for _ in curvatures), f"No curved surfaces found in {file_path}."
    assert any(0.0 < _.radius for _ in zmx_surfaces), \
        f"At least some radii are expected to be strictly positive, not {[f'{_.radius} {_.type}' for _ in zmx_surfaces]} in {file_path}."
    ...
    if file_path.parent.name != "long_wavelength":
        assert all(10e-9 <= _ <= 100e-6 for _ in optical_design.source.wavelengths), f"Unusual wavelengths found {optical_design.source.wavelengths} in {file_path}!"
    else:
        assert all(100e-6 <= _ <= 10e-3 for _ in optical_design.source.wavelengths), f"Unusual long wavelengths found {optical_design.source.wavelengths} in {file_path}!"
